...
float calculateShadow()
{
	...
	vec3 posws = (viewInverse*vec4(viewSpacePosition,1.0f)).xyz;
	...
	vec4 posLS = lightSpaceMatrices[layer] * vec4(posws, 1.0);
...
void main()
{
	...
	{ // Direct illumination
		direct_illumination_term = calculateDirectIllumiunation(viewDir, normalvs,base_color)*visibility;
	}
