...
namespace Acg {
	...
	struct Material
	{
		...
		Ref<Texture> m_shininess_texture;
		Ref<Texture> m_metalness_texture;
		Ref<Texture> m_fresnel_texture;
		Ref<Texture> m_emission_texture;
	};
	...
	void render(const Model* model, const Ref<Shader>& current_program, const bool submitMaterials = true);
} // namespace Acg