...
void MusicBuffer::Play()
{
	...
	MT_ASSERT(alGetError() == AL_NO_ERROR,"Error buffering for playback");

	/* Now queue and start playback! */
	alSourceQueueBuffers(p_Source, i, p_Buffers);
	alSourcePlay(p_Source);
	MT_ASSERT(alGetError() == AL_NO_ERROR, "Error starting playback");
...
void MusicBuffer::UpdateBufferStream()
{
	...
	while (processed > 0)
	{
		...
		{
			MT_ERROR("error buffering music data");
		}
	}
...
MusicBuffer::MusicBuffer(const char* filename)
{
	...
	if (!p_SndFile)
	{
		MT_ERROR("could not open provided music file -- check path");
	}
	...
	if (!p_Format)
	{
		sf_close(p_SndFile);
		p_SndFile = NULL;
		MT_ERROR("Unsupported channel count from file");
	}
